...
# Оптимізаційні параметри навчання
LR_NEURO_SYMBOLIC = 0.002
LR_PURE_NEURAL = 0.001
...
class CurvatureMLP(nn.Module):
    """Шукає ОКРЕМІ коефіцієнти викривлення alpha для кожного виміру (каналу) 
    за допомогою персептрона (MLP) БЕЗ стабілізуючого шару."""
...
class PureNeuralModel(nn.Module):
    """Чисто нейромережевий градієнтний граф: MLP (alphas) -> Restore -> MLP (coefs)."""
...
class Stage4UI:
    """Управління навчанням моделей, апроксимацією та генерацією аналітичних звітів."""
    ...
    def setup_ui(self):
        """Побудова візуальних компонентів керування вікна програми."""
        ...
        ttk.Button(ctrl_frame, text="4. Порівняти методы (Звіт)", command=self.compare_methods).grid(row=3, column=0, columnspan=2, pady=5, sticky=tk.EW)
    ...
    def log(self, message: str):
        """Записує події в текстове вікно інтерфейсу користувача."""
        self.txt_log.insert(tk.END, message + "\n")
        self.txt_log.see(tk.END)

    def thread_safe_log(self, message: str):
        """Забезпечує потокобезпечне логування з фонових задач."""
    ...
    def _generate_batch(self, batch_size: int, n_points: int, n_features: int, target_dim: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Генерує синтетичні нелінійні дані для тренувального кроку оптимізатора."""
    ...
    def prepare_training(self) -> bool:
        """Перевіряє передумови та конфігурує буфер історії перед запуском навчання."""
    ...
    def train_neuro_symbolic(self):
        """Ініціює ізольований потік розрахунку для гібридної архітектури."""
    ...
    def train_pure_neural(self):
        """Ініціює ізольований потік розрахунку для чистої нейромережі."""
        if not self.prepare_training(): 
            return
        threading.Thread(target=self._training_loop, args=('pure_neural',), daemon=True).start()

    def _training_loop(self, mode: str):
        """Уніфікована процедура градієнтного спуску та логування епох моделей."""
        ...
        self.thread_safe_log(f"\nНавчання {mode.upper()} завершено. Ви можете натиснути 'Застосувати'.")
        self.is_training = False

    def _build_plane_callback(self, coefs_attr: str) -> Callable[[np.ndarray, np.ndarray, int, int], np.ndarray]:
        """Генерує замикання обчислення висоти площини регресії за назвою атрибута коефіцієнтів."""
    ...
    def calculate_result(self):
        """Прогнозує параметри викривлення та будує 3D апроксимацію реальних даних."""
        ...
        if self.model_ns is not None:
            ...
            with torch.no_grad():
                ...
                self.log(f"Знайдено α для кожного виміру: {np.round(alphas, 4)}")
                
                restored_ns = np.zeros_like(data)
                for i in range(n_features):
                    restored_ns[:, i] = np.sign(data[:, i]) * (np.abs(data[:, i]) + STABILITY_EPSILON) ** (1.0 / alphas[i])
        ...
        if self.model_pn is not None:
            ...
            with torch.no_grad():
                ...
                self.log(f"Знайдено α для кожного виміру: {np.round(alphas.cpu().numpy()[0], 4)}")
    ...
    def _calculate_metrics(self, Z_true: np.ndarray, Z_pred: np.ndarray) -> Dict[str, float]:
        """Розраховує математичні та статистичні критерії точності прогнозу."""
    ...
    def compare_methods(self):
        """Виконує порівняльний аналіз та запускає генератор вікна статистичного звіту."""
        ...
        self._render_report_window(len(Z_true), X, Z_true, predict_z(self.app.stage4_ns_coefs), 
                                   predict_z(self.app.stage4_pn_coefs), metrics_ns, metrics_pn, t_stat, p_val)
    ...
    def _render_report_window(self, n_points: int, X: np.ndarray, Z_true: np.ndarray, Z_pred_ns: np.ndarray, 
        ...
        """Будує окреме графічне вікно та заповнює його аналітичними висновками."""
        ...
        try:
            ...
            if mlp_alphas is not None:
                ...
                rep_text.insert(tk.END, "1. ПЕРЕВІРКА ГІПОТЕЗИ ДЛЯ MLP (ВІДНОВЛЕННЯ ОКРЕМІХ α)\n" + cnn_report + "="*70 + "\n\n")
        except Exception as e:
            rep_text.insert(tk.END, f"[Помилка обчислення T-тесту для MLP: {e}]\n\n")
        
        # 2. Порівняння структурних параметрів
        ...
        # 4. Ступінь значущості параметрів площини
        if hasattr(self.app, "true_coefs"):
        ...
        if p_val < 0.05:
            ...
            rep_text.insert(tk.END, f"Статистично значущої різниці немає (P-value >= 0.05, t={t_stat:.3f}).\nОбидві нейромережі відпрацювали однаково ефективно.")
